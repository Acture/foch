...
function resolveProjectManifest(cfg) {
	const manifest = normalizePath(cfg.get('projectManifest'));
...
function shouldStartLanguageClient(targets, projectManifest) {
	return !!projectManifest || targets.length > 0 || hasOpenFochDocument();
}
...
async function activate(context) {
	...
	const projectManifest = resolveProjectManifest(cfg);
	...
	if (!shouldStartLanguageClient(targets, projectManifest)) {
		outputChannel.appendLine(
			'Foch is inactive in this workspace: configure fochLsp.projectManifest/modPaths/gamePath or open a workspace containing descriptor.mod.'
		);
		return;
	}
	...
	if (projectManifest) {
		env.FOCH_LSP_PROJECT_MANIFEST = projectManifest;
	}
	if (targets.length > 0) {
