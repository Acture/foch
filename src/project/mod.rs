...
pub struct ProjectImport {
	pub kind: ProjectImportKind,
	pub path: PathBuf,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum ProjectImportKind {
	DlcLoad,
}
...
pub struct ProjectMod {
	...
	#[serde(default = "default_project_mod_enabled")]
	pub enabled: bool,
	#[serde(default, skip_serializing_if = "Option::is_none")]
	pub position: Option<usize>,
}

fn default_project_mod_enabled() -> bool {
	true
}
...
impl Project {
	...
	fn search_paths(playset_root: &Path) -> Vec<PathBuf> {
		...
		// input::Config (steam_root_path / game_path / extra_ignore_patterns),
...
"#,
	...
	#[test]
	fn parses_project_manifest_config() {
		let config = Project::from_toml_str(
			r#"
[project]
game = "eu4"
...
"#,
		...
		assert_eq!(project.game, Some(Eu4));
		assert_eq!(project.imports.len(), 1);
		assert_eq!(project.imports[0].kind, ProjectImportKind::DlcLoad);
		assert_eq!(project.mods.len(), 2);
		assert_eq!(project.mods[0].id.as_deref(), Some("local_patch"));
		assert!(project.mods[0].enabled);
		assert_eq!(project.mods[1].steam_id.as_deref(), Some("2164202838"));
		assert_eq!(
			project.mods[1]
				.workshop_identity
				.as_ref()
		...
		assert!(!project.mods[1].enabled);
		assert_eq!(project.mods[1].position, Some(4));
	}
...
"#,
	...
	fn lookup_iteration_is_deterministic_across_processes() {
		let entries = Project::from_toml_str(
			r#"
[[resolutions]]
...
"#,
		...
		for (name, content, expected) in cases {
			let err = Project::from_toml_str(content).expect_err(name);
...
"#,
	...
	fn search_paths_excludes_engine_config_toml() {
		let temp = TempDir::new().expect("temp dir");
		let paths = Project::search_paths(temp.path());
		// Project (overrides/resolutions) 不能撞到 input::Config
		// 占用的 ~/.config/foch/config.toml 文件,否则两个 schema 互相 deny_unknown_fields。
		for path in &paths {
			assert!(
				!path.ends_with("config.toml"),
				"Project 不应扫描 config.toml 文件名(归 input::Config 所有);命中:{:?}",
				path
			);
		}
	}
...
"#,
		...
		for content in cases {
			let err =
				Project::from_toml_str(content).expect_err("multi-selector should be rejected");
			assert!(
