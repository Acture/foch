...
pub fn render_merge_report_text(report: &MergeReport) -> String {
	...
	lines.push(format!(
		"cross_file_noop_skipped_file_count: {}",
...
mod tests {
	...
	fn stale_vanilla_target(mod_id: &str) -> crate::model::StaleVanillaTargetDescriptor {
	...
	fn version_mismatch_finding(mod_id: &str) -> crate::model::VersionMismatchFinding {
		crate::model::VersionMismatchFinding {
	...
	fn dep_misuse_finding(
		...
		DepMisuseFinding {
			...
			evidence: crate::model::DepMisuseEvidence {
	...
	fn render_merge_report_text_includes_conflict_kinds() {
		let report = MergeReport {
			conflict_resolutions: vec![crate::model::MergeReportConflictResolution {
				...
				leaf_conflicts: vec![crate::model::LeafConflictDetail {
