...
/// output derived from its verified identity is committed.
#[must_use = "dropping the guard allows the installed snapshot to be replaced"]
#[derive(Debug)]
pub(crate) struct InstalledBaseSnapshotCommitGuard {
	_lock_file: fs::File,
}
...
) -> Result<InstalledBaseSnapshotCommitGuard, String> {
	let guard = InstalledBaseSnapshotCommitGuard {
		_lock_file: acquire_installed_base_snapshot_lock(
			game_key,
			game_version,
