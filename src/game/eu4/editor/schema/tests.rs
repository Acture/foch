...
/// P-687: the game reading a different value than the file states is an error.
#[test]
...
/// Trailing zeros read back as written, so the note drops to a warning: the
/// value survives, but it is not the precision the author appears to expect.
#[test]
fn value_coercion_warns_on_a_lossless_excess_decimal() {
	...
	assert!(
		...
		"the exact lowercase spelling is the boolean: {diagnostics:?}"
	);
}
