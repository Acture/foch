...
fn keyword_tables_match_cwt_helpers() {
	...
	let iterator_mismatches = iterator_keys()
		...
		.filter_map(|key| {
			...
			let cwt = schema_iterator_scope_type(engine, key);
	...
	let scope_changer_mismatches = scope_changer_keys()
		...
		.filter_map(|key| {
			...
			let cwt = schema_scope_changer_target_type(engine, key);
			(legacy != cwt).then(|| OptionScopeMismatch {
				key: key.to_string(),
				legacy,
				cwt,
	...
	let special_block_mismatches = special_block_keys()
		...
		.filter_map(|key| {
			...
			let cwt = schema_special_block_scope_kind(engine, key);
...
fn vanilla_corpus_matches_cwt_helpers() {
		...
		.filter(|entry| entry.path().extension().and_then(|ext| ext.to_str()) == Some("txt"))
	{
		...
		walk_keys(entry.path(), &parsed.ast.statements, &mut |key, line| {
			...
			let cwt_iterator = schema_iterator_scope_type(engine, key);
			...
			let cwt_scope_changer = schema_scope_changer_target_type(engine, key);
			...
			let cwt_special_block = schema_special_block_scope_kind(engine, key);
