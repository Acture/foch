...
fn container_scope_table_matches_cwt_helper() {
	...
	let mismatches = cases
		.iter()
		.filter_map(|case| {
			let cwt = schema_file_kind_container_scope_kind(
				engine,
				ScriptFileKind::new(case.file_kind),
				case.key,
			);
...
fn describe_cwt_fields(engine: &CwtQuery, file_kind: &str, key: &str) -> Vec<String> {
...
fn root_type_candidates<'e>(engine: &'e CwtQuery, file_kind: &str) -> Vec<&'e CompiledRoot> {
