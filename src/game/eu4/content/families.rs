...
use super::{
	...
	ContentFamilyPathMatcher, ContentFamilyScopePolicy, ContentLoadPolicy, DedupPolicy,
	...
	EU4_CONTENT_FAMILIES.get_or_init(|| {
		let mut families = vec![
			...
			ContentFamilyDescriptor::exact("map/random_names", "map/random/RandomLandNames.txt")
				.kind(ScriptFileKind::new("random_map_names"))
			...
			ContentFamilyDescriptor::exact("map/random/scenarios", "map/random/RNWScenarios.txt")
				.kind(ScriptFileKind::new("random_map_scenarios"))
			...
			ContentFamilyDescriptor::exact("common/technology", "common/technology.txt")
				.kind(ScriptFileKind::new("technology_groups"))
			...
			ContentFamilyDescriptor::exact("common/achievements", "common/achievements.txt")
				.kind(ScriptFileKind::new("achievements"))
...
impl Eu4 {
	...
	pub fn capabilities_for_root(&self, root_family: &str) -> Option<ContentFamilyCapabilities> {
		self.descriptor_for_root_family(root_family)
			.map(|descriptor| descriptor.capabilities)
	}

	pub fn family_id_for_root(&self, root_family: &str) -> Option<&'static str> {
		self.descriptor_for_root_family(root_family)
			.map(|descriptor| descriptor.id.as_str())
	}
}

pub fn eu4() -> &'static Eu4 {
	ensure_base_scopes_initialized();
	&EU4_PROFILE
}
...
pub fn eu4_content_family_for_root_family(
	...
	eu4().descriptor_for_root_family(root_family)
}
