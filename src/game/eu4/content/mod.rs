...
#[serde(transparent)]
pub struct ScriptFileKind(Arc<str>);
...
impl ContentFamilyDescriptorBuilder {
	pub fn kind(mut self, kind: ScriptFileKind) -> Self {
...
mod tests {
	use super::{
		BooleanMergePolicy, ContentFamilyDescriptor, ContentFamilyId, MergePolicies, ScriptFileKind,
	};
	...
	fn cwt_type_equality_is_string_based() {
		assert_eq!(ScriptFileKind::new("events"), ScriptFileKind::new("events"));
	}
