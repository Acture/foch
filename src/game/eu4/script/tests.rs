use super::{
	ScriptFileKind, build_inferred_callable_scope_map,
	build_semantic_index as raw_build_semantic_index, classify_script_file,
	collect_inferred_callable_masks, effective_alias_scope_mask_with_overrides,
	parse_script_file as raw_parse_script_file, scope_kind,
};
use crate::game::eu4::Eu4;
use crate::game::eu4::analysis::{AnalyzeOptions, analyze_visibility};
...
"#,
	...
	.expect("parsed event");
	let engine = crate::game::schema::query::CwtQuery::from_graph(
		&crate::game::schema::compile::CwtSchemaGraph::default(),
	);
	let mut index = crate::model::SemanticIndex::default();
...
fn extractor_for_returns_none_for_families_without_extractors() {
	...
	let descriptor = ContentFamilyDescriptor {
		...
		scope_policy: super::super::content::ContentFamilyScopePolicy {
