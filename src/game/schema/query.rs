...
use super::compile::{
	...
	CwtRuleField, CwtRuleValue, CwtSchemaGraph, CwtScope, CwtSeverity, CwtSubtype,
...
impl CompiledRulePack {
	...
	pub(crate) fn to_bytes(&self) -> Result<Vec<u8>, CwtLoadError> {
		bincode::serialize(self).map_err(|error| CwtLoadError::Codec {
			message: error.to_string(),
		})
	}

	pub(crate) fn from_bytes(bytes: &[u8]) -> Result<Self, CwtLoadError> {
...
impl CompiledTypeKeyFilter {
	fn from_schema(filter: &SchemaRootKeyFilter) -> Self {
		match filter {
			SchemaRootKeyFilter::Exact(values) => Self::Exact(values.clone()),
			SchemaRootKeyFilter::Exclude(values) => Self::Exclude(values.clone()),
		}
	}
...
fn bound_node(definition: &CompiledRoot, path: Vec<String>) -> SchemaBinding {
	...
	SchemaBinding::Bound {
		type_id: SchemaRootId::new(definition.name.clone()),
		node_id,
	}
}
