...
use super::error::{ParseError, ProjectionError};
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum ParadoxScalar<'source> {
...
pub(crate) enum ParadoxNode<'source> {
	Assignment {
...
impl ParadoxTree {
	pub fn parse(bytes: &[u8]) -> Result<Self, ParseError> {
		let source = Arc::<str>::from(decode_paradox_bytes(bytes).into_owned());
...
fn decode_paradox_bytes(bytes: &[u8]) -> Cow<'_, str> {
