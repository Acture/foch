...
#[derive(Clone, Debug, Eq, PartialEq)]
pub enum SchemaRootKeyFilter {
	Exact(Vec<String>),
	Exclude(Vec<String>),
}

impl SchemaRootKeyFilter {
...
impl SchemaRootDefinition {
	fn new(name: SchemaRootId) -> Self {
		Self {
			name,
			path: None,
			path_file: None,
...
pub struct CwtSchemaGraph {
	pub types: HashMap<SchemaRootId, SchemaRootDefinition>,
...
impl CwtSchemaGraph {
	pub(crate) fn from_paradox_tree(tree: &ParadoxTree) -> Self {
		Self::try_from_paradox_tree(tree).expect("projecting ParadoxTree into CwtSchemaGraph")
	}

	pub(crate) fn from_directory(dir: &Path) -> Result<Self, CwtLoadError> {
...
fn parse_type_key_filter(comments: &[String]) -> Option<SchemaRootKeyFilter> {
	let mut filter = None;
	for comment in comments {
		let text = comment.trim();
		...
		filter = Some(match operator {
			"<>" => SchemaRootKeyFilter::Exclude(values),
			"=" => SchemaRootKeyFilter::Exact(values),
