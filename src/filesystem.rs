...
impl FS {
	pub fn collect_files(&self) -> Result<(), Box<dyn std::error::Error>> {
		...
		for entry in std::fs::read_dir(&self.root)? {
			...
			let path = entry.path();
			if path.is_file() {
				let relative_path = path.canonicalize().expect("Failed to canonicalize").strip_prefix(&self.root).expect("Path should be relative to root").to_path_buf();
								...
								match create_kind {
									notify::event::CreateKind::File => {
										let path = event.paths.
											first().expect("Event should have at least one path")
											.canonicalize().expect("Path should be canonicalized")
											.strip_prefix(&root).expect("Path should be relative to root")
											.to_path_buf();
										let hash = None;
							...
							EventKind::Modify(modify_kind) => {
								match modify_kind {
									notify::event::ModifyKind::Data(_) => {
										let file = event.paths.first().unwrap().strip_prefix(&root).expect("Path should be relative to root").to_path_buf();
										let hash: Option<[u8; 32]> = None; // 这里可以计算文件的哈希值
										files_ref.lock().unwrap().insert(file, hash);
									}
									notify::event::ModifyKind::Name(_) => {
										let src = event.paths.first().unwrap().strip_prefix(&root).expect("Path should be relative to root").to_path_buf();
										files_ref.lock().unwrap().remove(&src);
										let dst = event.paths.get(1).and_then(|p| p.strip_prefix(&root).ok()).map(PathBuf::from).unwrap();
...
mod tests {
	...
	fn test_fs_watch() {
		...
		let mut fs = FS::builder().root(temp_dir.path().to_path_buf())
			.build();

		fs.watch().expect("Failed to start filesystem watcher");
		fs::write(temp_dir.path().join("test_file_1.txt"), b"Hello, World!").expect("Failed to create test file");
		fs.collect_files().expect("Failed to collect files");
		assert!(fs.files.lock().unwrap().contains_key(&PathBuf::from("test_file_1.txt")));

		fs::write(temp_dir.path().join("test_file_2.txt"), b"Hello, World!").expect("Failed to create test file");
		...
		while start.elapsed() < timeout {
			if fs.files.lock().unwrap().contains_key(&PathBuf::from("test_file_2.txt")) {
				found = true;
				break;
			}
