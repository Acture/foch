...
use super::{
	InputScriptCache, LoadedModSnapshot,
	mod_snapshot::{build_transient_mod_snapshot, load_or_build_mod_snapshot},
};
...
use crate::project::{Project, ProjectConfig, ProjectImportKind, ProjectMod};
...
pub enum InputResolveErrorKind {
	PlaylistFormat,
	Io,
}

#[derive(Clone, Debug)]
pub struct InputResolveError {
	pub kind: InputResolveErrorKind,
	pub path: PathBuf,
	pub message: String,
}
...
impl Error for InputResolveError {}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum InputTargetRole {
	Game,
	Mod,
}
...
pub struct InputTarget {
	pub path: PathBuf,
	pub role: InputTargetRole,
	pub mod_id: Option<String>,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ResolvedInputMod {
	pub mod_id: String,
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct InputResolveSummary {
...
fn load_manifest_source(
	...
	let manifest = Project::load_from_path(path).map_err(|err| InputResolveError {
		kind: InputResolveErrorKind::PlaylistFormat,
		path: path.to_path_buf(),
		message: err.to_string(),
	})?;
	let project = manifest.project.as_ref().ok_or_else(|| InputResolveError {
		kind: InputResolveErrorKind::PlaylistFormat,
		path: path.to_path_buf(),
...
fn input_error_from_playset_parse(err: crate::playset::ParseError) -> InputResolveError {
	InputResolveError {
		kind: if matches!(err.kind, ParseErrorKind::Format) {
			InputResolveErrorKind::PlaylistFormat
		} else {
...
fn apply_project_config_overrides(
	...
	if let Some(path) = project.game_path.as_ref() {
		let game = project.game.as_ref().ok_or_else(|| InputResolveError {
			kind: InputResolveErrorKind::PlaylistFormat,
...
fn playset_from_project_config(
	...
	source_root: &Path,
	manifest_path: &Path,
) -> Result<Playset, InputResolveError> {
	...
	for import in &project.imports {
		match import.kind {
			ProjectImportKind::DlcLoad => {
				...
				let imported =
					Playset::from_dlc_load(&import_path).map_err(input_error_from_playset_parse)?;
...
fn playset_entry_from_project_mod(
	...
	next_position: usize,
	manifest_path: &Path,
) -> Result<PlaysetEntry, InputResolveError> {
...
pub(crate) fn build_input_inventory(
	request: &InputRequest,
	include_game_base: bool,
) -> Result<InputInventory, InputResolveError> {
	build_input_inventory_for_paths(request, include_game_base, None)
}

pub fn resolve_input_summary(
	request: &InputRequest,
...
pub fn resolve_input_targets(
	request: &InputRequest,
	include_game_root: bool,
) -> Result<Vec<InputTarget>, InputResolveError> {
...
/// Resolve the complete ordered product input manifest for a input.
...
pub fn resolve_product_input_manifest(
	...
	let mods = entries
		...
		.map(|(index, entry)| {
			...
			.map_err(|error| InputResolveError {
				kind: InputResolveErrorKind::Io,
		...
		.collect::<Result<Vec<_>, InputResolveError>>()?;
...
pub(crate) fn build_input_inventory_for_paths(
	...
	let snapshot_filter = match FileFilter::new(playlist.game, &config.extra_ignore_patterns) {
		Ok(filter) => filter,
		Err(message) => {
			tracing::warn!(target: "crate::input::resolve", message, "falling back to filter without extra ignore globs");
	...
	for mod_item in &mods {
		validate_workshop_input(mod_item).map_err(|error| InputResolveError {
			kind: InputResolveErrorKind::Io,
			path: mod_item
				.root_path
				.clone()
...
pub(crate) fn resolve_input(
	request: &InputRequest,
	include_game_base: bool,
) -> Result<ResolvedInput, InputResolveError> {
	let inventory = build_input_inventory(request, include_game_base)?;
	resolve_input_from_inventory(inventory)
}

pub(crate) fn resolve_input_from_inventory(
...
mod tests {
	...
	fn request_for_manifest(path: &Path) -> InputRequest {
...
"#,
		...
		let summary = resolve_input_summary(&request).expect("resolve manifest");
		...
		let targets = resolve_input_targets(&request, true).expect("resolve targets");
		assert_eq!(targets.len(), 3);
		assert!(
			targets
				.iter()
				.any(|target| target.role == InputTargetRole::Game)
		);
	...
	#[cfg(unix)]
	#[test]
	fn input_summary_does_not_walk_mod_content() {
		use std::os::unix::fs::PermissionsExt;
...
"#,
		...
		let result = resolve_input_summary(&request_for_manifest(&manifest_path));
...
"#,
		...
		let summary =
			resolve_input_summary(&request_for_manifest(&manifest_path)).expect("resolve manifest");
		assert_eq!(summary.mods.len(), 1);
		assert_eq!(summary.mods[0].mod_id, "1001");
		assert_eq!(
			summary.mods[0].root_path.as_deref(),
...
"#,
		...
		let targets =
			resolve_input_targets(&request_for_manifest(&manifest_path), false).expect("targets");
		assert!(targets.is_empty());
	}

	#[test]
	fn input_product_input_manifest_reads_acf_not_mod_content() {
		let temp = TempDir::new().expect("tempdir");
...
"#,
		...
		.expect("build retained input inventory");
		...
		let input = resolve_input_from_inventory(inventory).expect("resolve input");
		assert_eq!(
			input.mods[0]
				.descriptor
				.as_ref()
