	...
	pub(crate) fn commit(mut self) -> Result<(), MergeError> {
		...
		let OutputTargetState::Directory(expected_identity) = current_target else {
			if let Err(error) = rename_output_noreplace(&staging_dir, &self.final_dir) {
				return Err(transaction_error(
					error,
					format!("failed to commit new output {}", self.final_dir.display()),
				));
			}
		...
		if let Err(error) = exchange_output_paths(&staging_dir, &self.final_dir) {
			return Err(transaction_error(
				error,
				format!(
					"failed to atomically commit output {}",
		...
		match inspect_output_target(&staging_dir) {
			Ok(OutputTargetState::Directory(identity)) if identity == expected_identity => {
				...
				self.cleanup_staging("committed output backup");
			}
			Ok(observed) => {
				self.staging_dir = None;
				return Err(MergeError::Io(io::Error::new(
					...
					format!(
						"merge output {} changed during atomic commit; the displaced target ({observed:?}) was preserved at {} and the staged output is now committed",
						self.final_dir.display(),
						staging_dir.display()
					),
				)));
			...
			Err(error) => {
				...
				return Err(MergeError::Io(io::Error::new(
					error_kind,
					format!(
						"merge output {} was committed, but the displaced target at {} could not be verified and was left untouched: {error}",
						self.final_dir.display(),
						staging_dir.display()
					),
				)));
...
fn rename_output_noreplace(_source: &Path, _target: &Path) -> io::Result<()> {
	Err(io::Error::new(
		io::ErrorKind::Unsupported,
		"atomic no-replace directory commit is unsupported on this platform",
	))
}
...
fn ensure_transaction_platform_supported() -> Result<(), MergeError> {
	Err(MergeError::Io(io::Error::new(
		io::ErrorKind::Unsupported,
		"transactional directory commit is unsupported on this platform",
	)))
}
