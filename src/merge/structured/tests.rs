...
fn structured_merge_matches_repeated_modifiers_by_tooltip_identity() {
	...
	assert!(outcome.conflicts().is_empty(), "{:?}", outcome.conflicts());
	let output = emit(outcome.resolved_ast().expect("commitable modifier merge"));
...
#[test]
fn structured_merge_preserves_orphan_control_flow_but_withholds_commit() {
	let source = parse(
		"scripted_effect = {\n\
...
fn event_merge_keeps_distinct_complete_constructor_chains_independent() {
	...
	assert!(outcome.conflicts().is_empty(), "{:?}", outcome.conflicts());
	let resolved = outcome.resolved_ast().expect("commitable event AST");
...
fn event_merge_matches_replaced_open_chains_by_sequence_context() {
	...
	assert!(outcome.conflicts().is_empty(), "{:?}", outcome.conflicts());
	let output = emit(outcome.resolved_ast().expect("commitable event AST"));
	for retained in [
		"ge_marker = yes",
		"has_government_attribute = republican_virtues",
...
fn eu4_ages_reducer_retains_the_stronger_value_against_a_one_sided_change() {
	...
	assert_eq!(
		...
		vec![ClausewitzScalarReduction {
			...
			inputs: vec![
				(crate::merge::kernel::RevisionId::LEFT, "50".to_string()),
				(crate::merge::kernel::RevisionId::RIGHT, "35".to_string()),
			],
			output: "50".to_string(),
		}]
	);
