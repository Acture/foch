...
mod tests {
	...
	use crate::merge::kernel::{ConflictResolution, DeltaOperation, RevisionId};
		...
		for (conflict, view) in probe.conflicts.iter().zip(&handler.views) {
			...
			for (source, candidate) in sources.zip(&view.candidates) {
				assert_eq!(
					...
					matches!(
						source,
						crate::merge::kernel::SourceNodeRef::Tombstone { .. }
					),
	...
	fn node_with_a_mismatched_partition_root_renders_as_unrenderable() {
		...
		let wrong_root_hash = conflict
			...
			.input()
			.root_hash;
		let crate::merge::kernel::SourceNodeRef::Node {
		...
		conflict.candidates[source_index] = crate::merge::kernel::SourceNodeRef::Node {
