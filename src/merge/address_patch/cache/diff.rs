...
impl ModDiffCache {
	pub fn open(cache_dir: &Path) -> Self {
		...
		if let Err(error) = ensure_generation(cache_dir, MOD_DIFF_CACHE_VERSION) {
			tracing::warn!(
...
fn default_mod_diff_cache_dir() -> PathBuf {
...
mod tests {
	use super::*;
	use crate::game::eu4::script::parser::{AstValue, ScalarValue, Span, SpanRange};
	...
	fn mod_diff_cache_open_preserves_old_generations_flat_and_temporary_items() {
		let input = cache_dir("mod-diff-generation-lifecycle");
		let layer_root = input.join("diffs");
		...
		let unrelated_entry = input.join("dag-base").join("v1").join("keep.bin");
