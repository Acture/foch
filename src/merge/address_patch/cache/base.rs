...
impl DagBaseCache {
	pub fn open(cache_dir: &Path) -> Self {
		...
		if let Err(error) = ensure_generation(cache_dir, DAG_BASE_CACHE_VERSION) {
			tracing::warn!(
...
fn default_dag_base_cache_dir() -> PathBuf {
...
mod tests {
	...
	fn dag_base_cache_open_preserves_old_generations_flat_and_temporary_items() {
		let input = cache_dir("dag-base-generation-lifecycle");
		let layer_root = input.join("dag-base");
		...
		let unrelated_entry = input.join("diffs").join("v1").join("keep.bin");
