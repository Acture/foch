...
use crate::game::eu4::script::{ParsedScriptFile, parse_script_file};
...
pub(crate) struct DagMergeInputRequest<'a> {
	...
	pub script_cache: Option<&'a InputScriptCache>,
}
...
pub(crate) fn contributor_mod_hashes(
	...
	let by_mod: HashMap<ModId, &ResolvedInputContributor> = contributors
		.iter()
...
mod tests {
	...
	fn verified_cache_error_never_falls_back_to_direct_parsing() {
		...
		let cache = InputScriptCache::default();
