...
fn record_semantic_top_level_key(
	...
	let tree = if node.revision == crate::merge::kernel::RevisionId::BASE {
		&partition.base_tree
...
mod tests {
	...
	fn compute(
		...
		contribs: Vec<ResolvedInputContributor>,
		vanilla_source: Option<&str>,
	...
	fn rendered(statements: &[AstStatement]) -> String {
		crate::game::eu4::script::emit::emit_clausewitz_statements(statements)
			.expect("emit statements")
	}
	...
	fn resolved_branch_cache_invalidates_when_merge_policy_changes() {
		use crate::game::eu4::content::ScalarMergePolicy;
	...
	fn event_merge_policies() -> MergePolicies {
		MergePolicies {
			...
			scalar: crate::game::eu4::content::ScalarMergePolicy::LastWriter,
