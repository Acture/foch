...
#[test]
fn public_merge_facade_exposes_only_current_backend_names() {
	let crate_facade = include_str!("../lib.rs");
	let merge_facade = include_str!("mod.rs");
	assert!(crate_facade.contains("pub mod merge;"));
	assert!(
		merge_facade.contains("pub use kernel_adapter::{MergeBackendDescriptor, MergeBackendId};")
	);
	assert_source_excludes(
		"merge/mod.rs",
		merge_facade,
...
fn commit_consumes_frozen_artifacts_without_reanalysis() {
	...
	assert!(include_str!("output/materialize/output_transaction.rs").contains("fn commit("));
}
