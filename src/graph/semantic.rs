...
pub const SEMANTIC_GRAPH_PROGRESS_TARGET: &str = "crate::graph::progress";
...
pub(crate) fn run_semantic_graph_with_options(
	...
	let input = run_progress_stage(
		&family_id,
		"resolve input",
		|| resolve_input(&request, options.include_game_base).map_err(|err| err.message),
		summarize_input,
	)
	.map_err(boxed_err)?;
...
fn summarize_input(input: &ResolvedInput) -> String {
	let enabled_mod_count = input.mods.iter().filter(|item| item.entry.enabled).count();
	let inventory_file_count = input.file_inventory.len();
	let contributor_file_count = input
		.file_inventory
		.values()
	...
	tracing::debug!(
		...
		enabled_mod_count,
		has_base_game = input.installed_base_snapshot.is_some(),
		"semantic graph input resolved"
	);
	format!(
...
fn definition_reference_family(key: &str) -> Option<&'static str> {
	match key {
		...
		"idea_group_definition" => Some("common/ideas"),
		"diplomatic_action_definition" => Some("common/diplomatic_actions"),
		"scripted_trigger_definition" => Some("common/scripted_triggers"),
...
fn symbol_kind_text(kind: crate::model::SymbolKind) -> &'static str {
	match kind {
		crate::model::SymbolKind::Event => "event",
		crate::model::SymbolKind::Decision => "decision",
		crate::model::SymbolKind::ScriptedEffect => "scripted_effect",
		crate::model::SymbolKind::ScriptedTrigger => "scripted_trigger",
		crate::model::SymbolKind::DiplomaticAction => "diplomatic_action",
		crate::model::SymbolKind::TriggeredModifier => "triggered_modifier",
	}
}
...
mod tests {
	...
	fn classifier_keeps_same_family_definitions_separate_from_cross_family_refs() {
		...
		assert_eq!(
			classify_reference_target("common/cultures", "culture_definition"),
			ReferenceTargetClass::SameFamily
		);
		assert_eq!(
			classify_reference_target("common/advisortypes", "advisor_type_definition"),
			ReferenceTargetClass::SameFamily
		);
		assert_eq!(
		...
		assert_eq!(
			classify_reference_target("common/event_modifiers", "event_modifier_definition"),
			ReferenceTargetClass::SameFamily
		);
		assert_eq!(
			classify_reference_target(
				"common/province_triggered_modifiers",
				"province_triggered_modifier_definition"
			),
			ReferenceTargetClass::SameFamily
		);
		assert_eq!(
			classify_reference_target("common/cb_types", "cb_type_definition"),
			ReferenceTargetClass::SameFamily
		);
		assert_eq!(
			classify_reference_target("common/ideas", "idea_group_definition"),
			ReferenceTargetClass::SameFamily
		);
		assert_eq!(
		...
		assert_eq!(
			classify_reference_target("common/scripted_triggers", "scripted_trigger_definition"),
			ReferenceTargetClass::SameFamily
		);
		assert_eq!(
			classify_reference_target(
	...
	#[test]
	fn semantic_mode_requires_family_before_input_resolution() {
		...
		let result = run_semantic_graph_with_options(
			...
			GraphBuildOptions {
				...
				scope: GraphScopeSelection::Input,
