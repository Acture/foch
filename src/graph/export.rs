...
pub fn run_graph_with_options(
	...
	let input_calls = build_input_calls_graph(&state);
	let input_deps = build_input_mod_deps_graph(&state, &request);
	let input_def_deps = build_input_definition_deps_artifact(&state, &options.definition_kinds);
	...
	match options.scope {
		GraphScopeSelection::Input | GraphScopeSelection::All => {
			write_graph_pair(
				&out_dir.join("input"),
				"calls",
				&input_calls,
				render_calls_dot(&input_calls),
				options.format,
			)?;
			write_graph_pair(
				&out_dir.join("input"),
				"mod-deps",
				&input_deps,
				render_mod_deps_dot(&input_deps),
				options.format,
			)?;
			write_graph_pair(
				&out_dir.join("input"),
				"definition-deps",
				&input_def_deps,
				render_definition_deps_dot(&input_def_deps),
				options.format,
			)?;
			summary.input_written = true;
		}
		_ => {}
	}
	...
	{
		let base_calls = filter_calls_graph_by_mod(&input_calls, base_mod_id);
		let base_deps = filter_mod_deps_graph_by_mod(&input_deps, base_mod_id);
		let base_def_deps = filter_definition_deps_graph_by_mod(&input_def_deps, base_mod_id);
		write_graph_pair(
	...
	if matches!(
		...
		for mod_id in mod_ids {
			let calls = filter_calls_graph_by_mod(&input_calls, &mod_id);
			let deps = filter_mod_deps_graph_by_mod(&input_deps, &mod_id);
			let def_deps = filter_definition_deps_graph_by_mod(&input_def_deps, &mod_id);
			write_graph_pair(
	...
	if let Some(root) = options.root {
		let tree = filter_calls_graph_by_root(&input_calls, &root);
...
fn build_input_calls_graph(state: &crate::check::runtime::RuntimeState) -> CallsGraphArtifact {
...
fn build_input_mod_deps_graph(
	...
	let input = resolve_input(request, state.base_game_mod_id.is_some())
...
fn build_input_definition_deps_artifact(
	state: &crate::check::runtime::RuntimeState,
...
mod definition_deps_tests {
	use super::*;
	use crate::check::runtime::RuntimeState;
	use crate::model::{
	...
	fn state_with(
		...
		let semantic_index = crate::model::SemanticIndex {
			definitions,
			references,
			..Default::default()
		};
	...
	fn filter_definition_deps_by_mod_keeps_provider_node_and_inbound_edges() {
		...
		let artifact = build_input_definition_deps_artifact(&state, &[]);
	...
	fn definition_kinds_filter_drops_unrelated_kinds() {
		...
		let artifact = build_input_definition_deps_artifact(&state, &[SymbolKind::Event]);
