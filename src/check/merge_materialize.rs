...
pub(crate) fn materialize_merge_internal(
	...
	let plan = match resolve_workspace(&request, options.include_game_base) {
		...
		Err(_) => crate::check::run_merge_plan_with_options(
			request.clone(),
			MergePlanOptions {
...
fn find_contributor_path<'a>(
	...
	winner: &crate::check::model::MergePlanContributor,
) -> Option<&'a PathBuf> {
	contributors
		.iter()
...
mod tests {
	...
	use crate::check::model::{
		CheckRequest, MERGE_PLAN_ARTIFACT_PATH, MERGE_REPORT_ARTIFACT_PATH,
