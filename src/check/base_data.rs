...
#[derive(Debug)]
pub(crate) struct BaseBuildObserver {
	emit_progress: bool,
	started_at: u64,
...
impl BaseBuildObserver {
	pub(crate) fn stderr(game_key: &str) -> Self {
		Self::new(game_key, true)
	}

	pub(crate) fn silent(game_key: &str) -> Self {
	...
	pub(crate) fn set_game_version(&mut self, game_version: &str) {
		self.profile.game_version = game_version.to_string();
	}

	pub(crate) fn set_count(&mut self, key: impl Into<String>, value: usize) {
		self.profile.counts.insert(key.into(), value as u64);
	}

	pub(crate) fn set_inventory_file_count(&mut self, count: usize) {
	...
	pub(crate) fn set_parse_stats(&mut self, parse_stats: ParseFamilyStats) {
		self.profile.parse_stats = parse_stats;
	}

	pub(crate) fn set_encoded_size_bytes(&mut self, size: usize) {
	...
	pub(crate) fn set_encoded_sections(&mut self, sections: Vec<BaseEncodedSectionProfile>) {
		self.profile.encoded_sections = sections;
	}

	pub(crate) fn run_stage<T, F>(&mut self, name: &str, f: F) -> Result<T, String>
	where
	...
	pub(crate) fn finish(mut self) -> BaseBuildProfile {
...
pub(crate) fn build_base_snapshot_with_observer(
	game: &Game,
	game_root: &Path,
	game_version: Option<&str>,
	...
	let discovered_documents = observer.run_stage("discover_documents", |counts| {
