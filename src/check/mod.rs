...
use crate::check::runtime::{build_overlap_findings, build_runtime_state_from_input};
use crate::game::eu4::analysis::rules::{
...
use crate::game::eu4::analysis::{AnalyzeOptions, analyze_visibility_with_vanilla_index};
use crate::game::eu4::base::vanilla_index::{VanillaSymbolIndex, VanillaSymbolSource};
...
pub const CHECK_PROGRESS_TARGET: &str = "crate::check::progress";
...
impl VanillaSymbolSource for ResolvedInput {
...
pub fn run_checks(request: InputRequest) -> CheckResult {
	run_checks_with_options(request, CheckOptions::default())
}

pub fn run_checks_with_options(request: InputRequest, options: CheckOptions) -> CheckResult {
	let mut result = CheckResult::default();

	let resolved = run_progress_stage(
		"resolve input",
		|| resolve_input(&request, options.include_game_base),
		|res| match res {
			Ok(input) => {
	...
	let runtime_overlap_findings = if options.analysis_mode == AnalysisMode::Semantic {
		let runtime_overlap = run_progress_stage(
			"runtime overlap",
			|| {
				build_runtime_state_from_input(&resolved)
