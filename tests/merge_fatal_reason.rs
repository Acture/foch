//! Integration test: a FATAL merge caused by failed input resolution must
...
use foch::merge::{MergeAnalysisOptions, run_merge_with_options};
...
fn fatal_merge_surfaces_resolve_error_reason() {
	...
	// `resolve_input` returns the "missing installed base data" error.
