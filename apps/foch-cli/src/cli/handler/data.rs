		...
		.run_stage(
			...
			|counts: &mut std::collections::BTreeMap<String, u64>| {
				if args.install {
					...
					let coverage_bytes =
						std::fs::metadata(installed.install_dir.join(INSTALLED_COVERAGE_FILE_NAME))
							.map(|item| item.len())
							.unwrap_or_default();
...
fn handle_data_list(args: &DataListArgs) -> HandlerResult {
	let entries = list_installed_base_data()
		.map_err(|err: String| -> Box<dyn std::error::Error> { err.into() })?;
	if args.json {
...
fn parse_game(value: &str) -> Result<Eu4, Box<dyn std::error::Error>> {
	Eu4::from_key(value).ok_or_else(|| format!("unsupported game key: {value}").into())
}
