...
use foch::model::{
	MERGE_EXECUTION_ATTESTATION_SCHEMA, MERGE_REPORT_ARTIFACT_PATH, MergeBackendId, MergeReport,
	MergeReportBaseSnapshot, MergeReportScope, MergeReportStatus, PRODUCT_INPUT_DIGEST_ALGORITHM,
	PRODUCT_INPUT_MANIFEST_SCHEMA, PRODUCT_INPUT_PROFILE,
};
use foch::project::{Project, ProjectConfig, ProjectMod};

use crate::merge_quality::dataset::{EngineArtifactIdentity, MeasurementScope};
...
impl ProductCacheEnvironment {
	...
	fn apply_to(&self, command: &mut Command) {
		command.env(
			foch::platform::cache_store::CACHE_ROOT_ENV,
			self.root.path(),
		);
...
impl ProductMeasurementRunner {
	...
	fn run_product_command(
		...
		write_project_manifest(&manifest_path, request)?;
	...
	fn run_product_merge(
		...
		if let Some(report) = report {
			...
			if !is_committable_product_status(report.status) {
...
fn is_committable_product_status(status: MergeReportStatus) -> bool {
	matches!(
		status,
...
fn validate_product_attestation(
	...
	if execution.backend != MergeBackendId::GumtreePcsNway {
		return Err("product merge report did not attest the gumtree-pcs-nway backend".to_string());
	}
...
fn write_project_manifest(path: &Path, request: &MeasurementRequest) -> Result<(), String> {
	let mods = request
		.case
		.referenced_mods
		.iter()
	...
	let encoded = toml::to_string_pretty(&manifest)
		.map_err(|error| format!("failed to encode product project manifest: {error}"))?;
	fs::write(path, encoded)
		.map_err(|error| format!("failed to write product project manifest: {error}"))
}
...
mod tests {
	...
	fn cache_gate_rejects_an_override_and_applies_no_cap() {
		...
		assert_eq!(
			command_env(&command, foch::platform::cache_store::CACHE_ROOT_ENV),
	...
	#[test]
	fn only_ready_or_partial_success_reports_are_committable() {
		assert!(is_committable_product_status(MergeReportStatus::Ready));
		assert!(is_committable_product_status(
			MergeReportStatus::PartialSuccess
		));
		assert!(!is_committable_product_status(MergeReportStatus::Blocked));
		assert!(!is_committable_product_status(MergeReportStatus::Fatal));
	}
	...
	fn product_attestation_requires_exact_execution_and_input_identity() {
		let mut report = MergeReport {
			execution: Some(foch::model::MergeExecutionAttestation {
		...
		report.execution.as_mut().unwrap().backend = MergeBackendId::AddressPatch;
		assert!(validate_product_attestation(&report, &request, false).is_err());
		report.execution.as_mut().unwrap().backend = MergeBackendId::GumtreePcsNway;
