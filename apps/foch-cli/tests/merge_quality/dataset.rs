...
use crate::merge_quality::corpus::WorkshopProvenance;
...
impl MeasurementCohortKey {
	...
	pub const fn backend(&self) -> Option<MergeBackendId> {
		match self {
			Self::OrchestratorBoundV1 { .. } => None,
			Self::EngineArtifactV2 { backend, .. } => Some(*backend),
		}
	}
...
impl DatasetPaths {
	...
	/// Create only the append-only streams and compact-evidence paths used by
	/// live Workshop measurement.
...
mod tests {
	...
	#[test]
	fn workshop_layout_initialization_is_repeatable_and_current_only() {
		let temp = tempfile::tempdir().unwrap();
		...
		assert!(paths.measurements.is_file());
		assert!(paths.file_results.is_file());
		assert!(!paths.root.join("dataset.json").exists());
		assert!(!paths.root.join("object_records.jsonl").exists());
		assert!(!paths.root.join("snapshots.jsonl").exists());
	}
	...
	fn v2_identity() -> MeasurementIdentityV2 {
		MeasurementIdentityV2 {
			...
			backend: MergeBackendId::AddressPatch,
	...
	fn v2_identity_sensitivity_matrix_covers_every_semantic_input() {
		...
		structured.backend = MergeBackendId::GumtreePcsNway;
		...
		assert_eq!(
			serde_json::to_value(structured).unwrap()["backend"],
			"gumtree-pcs-nway"
		);
	}
