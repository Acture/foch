...
struct IteratorScopeRule {
	scope_type: ScopeType,
	keys: &'static [&'static str],
}

const ITERATOR_SCOPE_RULES: &[IteratorScopeRule] = &[
	IteratorScopeRule {
...
pub fn iterator_scope_type(key: &str) -> Option<ScopeType> {
	for rule in ITERATOR_SCOPE_RULES {
		if rule.keys.contains(&key) {
			return Some(rule.scope_type);
		}
	}
	None
}
...
#[cfg(test)]
mod tests {
	use super::{ITERATOR_SCOPE_RULES, iterator_scope_type};
	...
	#[test]
	fn iterator_scope_rules_are_non_overlapping() {
		let mut seen = HashSet::new();
		for rule in ITERATOR_SCOPE_RULES {
			assert!(!rule.keys.is_empty());
			for key in rule.keys {
				assert!(seen.insert(*key), "duplicate iterator rule for {key}");
				assert_eq!(iterator_scope_type(key), Some(rule.scope_type), "{key}");
			}
		}
	}
}
