...
use foch_core::model::{ProductInputManifest, ProductInputMod};
use foch_core::utils::steam::SteamId;

use crate::config::{WorkshopCatalog, WorkshopItemVersion};
use crate::corpus::Case;
use crate::dataset::{GameInputIdentityV2, InputVersionRecord, WorkshopInputVersionV2};
...
impl ResolvedWorkshopCase {
	...
	pub fn input_version(
		...
		Ok(InputVersionRecord::new(
			...
			GameInputIdentityV2 {
				app_id: SteamId::new(u64::from(crate::config::EU4_APPID)),
...
mod tests {
	...
	fn synthetic_item(workshop_id: &str) -> ResolvedWorkshopItem {
		ResolvedWorkshopItem {
			install: WorkshopItemVersion {
				identity: foch_core::utils::steam::WorkshopInstallIdentity {
	...
	fn fixed_workshop_product_manifest_is_exactly_fourteen_cases() {
		let manifest = WorkshopCaseManifest::from_json(include_str!(
			"../tests/fixtures/workshop-product-cases-v2.json"
		))
		.unwrap();
			...
			r#""AppWorkshop"
{
	...
	"WorkshopItemsInstalled"
	{
		...
		.unwrap();
		let catalog =
			WorkshopCatalog::from_override(crate::config::EU4_APPID, &content_root, &manifest_path)
				.unwrap();
