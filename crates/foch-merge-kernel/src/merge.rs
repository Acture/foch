...
use crate::{
	ChildOrder, ClassId, ClassMapping, ConflictKind, Matching, MergeOutcome, MergeTimings, NodeId,
	NormalizedNode, NormalizedTree, RevisionClass, RevisionId, RevisionNode, SourceSet,
	StructuralConflict, TreeMatcher, TreeNode,
};
...
pub fn three_way_merge(
	...
	let mut states = select_classes(&trees, &mapping, &mut conflicts);
	...
	let provenance = sources_preorder
		...
		.collect();
	let pcs_ns = nanos(pcs_started.elapsed());
	...
	for class in mapping.classes() {
		...
		let keep = match (base, left, right) {
			...
			(Some(base_id), Some(present), None) => {
				let content_unchanged = trees.base.node(base_id).unwrap().subtree_hash
					== trees.left.node(present).unwrap().subtree_hash;
			...
			(Some(base_id), None, Some(present)) => {
				let content_unchanged = trees.base.node(base_id).unwrap().subtree_hash
					== trees.right.node(present).unwrap().subtree_hash;
		...
		let selected = select_revision_node(trees, class, mapping, conflicts);
...
fn relative_order_changed(
	...
	if base_node.child_order != ChildOrder::Ordered
